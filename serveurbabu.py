...
def send_file_route():
    ...
    messages = append_message(session["username"], receiver, f"[{file_type}]: {filename}")
    socketio.emit("new_message", {"sender": session["username"], "text": f"[{file_type}]", "url": url, "type": file_type, "date": messages["date"]}, room=receiver)
    socketio.emit("new_message", {"sender": session["username"], "text": f"[{file_type}]", "url": url, "type": file_type, "date": messages["date"]}, room=session["username"])
